...
def tutorial_main():
    ...
    pipeline = Gst.parse_launch(" \
        ...
        hailonet name=inference_hailonet hef-path=./resources/yolov8s_h8l.hef batch-size=2 nms-score-threshold=0.3 nms-iou-threshold=0.45 output-format-type=HAILO_FORMAT_TYPE_FLOAT32 force-writable=true ! \
        hailofilter name=inference_hailofilter so-path=./resources/libyolo_hailortpp_postprocess.so qos=false ! \
        hailooverlay name=hailo_display_hailooverlay ! \
        videoconvert name=hailo_display_videoconvert n-threads=2 qos=false ! \
        fpsdisplaysink name=hailo_display video-sink=xvimagesink sync=true text-overlay=false signal-fps-measurements=true \
    ")
